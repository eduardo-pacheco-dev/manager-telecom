<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6">
    {{-- Page header --}}
    <x-ui.page-header
        :title="__('Configurações de Colaboradores')"
        :subtitle="__('Gerencie as categorias, cargos e departamentos disponíveis nos formulários')"
        :breadcrumbs="[
    ...
    >
        <flux:button href="{{ route('colaboradores.index') }}" wire:navigate variant="ghost" icon="arrow-left">
            {{ __('Voltar') }}
        </flux:button>
    ...
    <div class="animate-fade-in-up flex gap-1.5 overflow-x-auto rounded-2xl border border-zinc-200 bg-white/90 p-1.5 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-zinc-900/90">
        @php
            $abas = [
                'categorias' => ['identification', __('Categorias')],
                'cargos' => ['briefcase', __('Cargos')],
                'departamentos' => ['building-office', __('Departamentos')],
            ];
        @endphp
    ...
    <div class="animate-fade-in-up overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-white/10 dark:bg-white/[0.03]" style="animation-delay: 180ms">
        <div class="flex items-center justify-between border-b border-zinc-200 px-5 py-3.5 dark:border-white/10">
            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                {{ $aba === 'categorias'
                    ? __('Categorias de colaboradores cadastradas')
                    : ($aba === 'cargos' ? __('Cargos cadastrados') : __('Departamentos cadastrados')) }}
            </p>
        </div>
            ...
            @empty
                <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                    ...
                    <p class="mt-1 max-w-sm text-sm text-zinc-500 dark:text-zinc-400">
                        {{ __('Crie registros para disponibilizá-los nos formulários de colaboradores.') }}
                    </p>
        ...
        <div class="space-y-5">
            <div>
                ...
                <flux:text class="mt-2">
                    {{ __('O registro aparece nos formulários de colaboradores. Registros inativos ficam ocultos na seleção.') }}
                </flux:text>
            </div>
            ...
            <div class="space-y-4">
                <flux:field>
                    ...
                    <flux:input wire:model="nome" type="text" required placeholder="{{ __('Ex.: Engenheiro de Redes') }}" />
