<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6">
    ...
    <x-ui.page-header
        :title="__('Serviços')"
        :subtitle="__('Gerencie os serviços da empresa')"
        :breadcrumbs="[
            ['label' => __('Gestão'), 'href' => null],
            ['label' => __('Serviços'), 'href' => null],
        ]"
    >
        <x-slot:titleBadge>
            <flux:button
                href="{{ route('servicos.configuracoes') }}"
                wire:navigate
                variant="ghost"
                size="sm"
                icon="cog-6-tooth"
    ...
    <x-ui.table
        ...
        :first-item="$servicos->firstItem()"
        :last-item="$servicos->lastItem()"
    ...
    >
            ...
            @if ($servicos->total() > 0)
                <th scope="col" class="px-5 py-3 text-left">
                    <input
                        ...
                        :checked="count(array_intersect($this->selecionados, $servicos->pluck('id')->all())) === $servicos->count()"
                ...
                <th scope="col" class="px-4 py-3 text-left">
                    <x-ui.sortable-header field="nome" :label="__('Serviço')" :sort-field="$sortField" :sort-direction="$sortDirection" />
                </th>
                <th scope="col" class="px-4 py-3 text-left">
        ...
        @forelse ($servicos as $servico)
            <x-servicos.row :servico="$servico" />
        @empty
            <tr wire:key="servico-empty">
                <td colspan="6">
                    <x-ui.empty
                        icon="wrench-screwdriver"
                        :title="__('Nenhum serviço encontrado')"
                    ...
                    >
                        ...
                        @else
                            <flux:button href="{{ route('servicos.create') }}" wire:navigate variant="primary" size="sm" icon="plus">
        ...
        <x-slot:footer>
            <x-ui.pagination :paginator="$servicos" show-per-page :per-page="$perPage" />
        </x-slot:footer>
    </x-ui.table>

    {{-- Delete confirmation modal --}}
    <x-servicos.delete-modal
        :servico-para-excluir="$servicoParaExcluir"
        :servico-alvo="$this->servicoAlvo"
    />

    {{-- Import modal --}}
    <x-servicos.import-modal
