<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6">
    ...
    <x-ui.page-header
        ...
        :subtitle="__('Gerencie as categorias disponíveis nos formulários')"
        :breadcrumbs="[
    ...
    <div class="animate-fade-in-up overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-white/10 dark:bg-white/[0.03]" style="animation-delay: 180ms">
        <div class="flex items-center justify-between border-b border-zinc-200 px-5 py-3.5 dark:border-white/10">
            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                {{ __('Categorias de serviços cadastradas') }}
            </p>
        </div>

        <div class="flex flex-col">
            @forelse ($this->itens as $item)
                <div wire:key="categoria-{{ $item->id }}" class="group flex items-center gap-4 border-b border-zinc-100 px-5 py-4 transition-colors last:border-b-0 hover:bg-zinc-50/80 dark:border-white/5 dark:hover:bg-white/[0.02]">
                    ...
                    @if ($item->ativo)
                        <span class="shrink-0 rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:text-emerald-400">{{ __('Ativa') }}</span>
                    @else
                        <span class="shrink-0 rounded-full bg-zinc-500/10 px-2.5 py-1 text-xs font-medium text-zinc-500 dark:text-zinc-400">{{ __('Inativa') }}</span>
                    @endif
                    ...
                    <div class="flex shrink-0 items-center gap-1">
                        ...
                        <flux:button
                            ...
                            wire:confirm="{{ __('Remover esta categoria?') }}"
                            size="sm"
                            variant="ghost"
                            icon="trash"
                            :title="__('Excluir')"
            ...
            @empty
                <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                    ...
                    <p class="mt-1 max-w-sm text-sm text-zinc-500 dark:text-zinc-400">
                        {{ __('Crie categorias para disponibilizá-las nos formulários de serviços.') }}
                    </p>
                    <flux:button wire:click="abrirNovo" variant="primary" size="sm" icon="plus" class="mt-5">
                        {{ __('Cadastrar primeira categoria') }}
                    </flux:button>
                </div>
            @endforelse
        </div>
    ...
    <flux:modal wire:model="showModal" class="max-w-md">
        <div class="space-y-5">
            <div>
                <flux:heading level="2">{{ $itemId ? __('Editar categoria') : __('Nova categoria') }}</flux:heading>
                <flux:text class="mt-2">
                    {{ __('A categoria aparece nos formulários de serviços. Categorias inativas ficam ocultas na seleção.') }}
                </flux:text>
            </div>
            ...
            <div class="space-y-4">
                <flux:field>
                    ...
                    <flux:input wire:model="nome" type="text" required placeholder="{{ __('Ex.: Instalação') }}" />
                ...
                <flux:switch wire:model="ativo" :label="__('Categoria ativa')" />
            </div>
