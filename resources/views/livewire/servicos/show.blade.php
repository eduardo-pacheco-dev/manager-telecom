@php
    $categoriaStyles = [
        'Instalação' => ['badge' => 'bg-sky-500/10 text-sky-700 ring-1 ring-inset ring-sky-500/20 dark:bg-sky-400/10 dark:text-sky-300 dark:ring-sky-400/20', 'dot' => 'bg-sky-500 dark:bg-sky-400'],
        'Manutenção' => ['badge' => 'bg-violet-500/10 text-violet-700 ring-1 ring-inset ring-violet-500/20 dark:bg-violet-400/10 dark:text-violet-300 dark:ring-violet-400/20', 'dot' => 'bg-violet-500 dark:bg-violet-400'],
        'Suporte' => ['badge' => 'bg-emerald-500/10 text-emerald-700 ring-1 ring-inset ring-emerald-500/20 dark:bg-emerald-400/10 dark:text-emerald-300 dark:ring-emerald-400/20', 'dot' => 'bg-emerald-500 dark:bg-emerald-400'],
        'Configuração' => ['badge' => 'bg-amber-500/10 text-amber-700 ring-1 ring-inset ring-amber-500/20 dark:bg-amber-400/10 dark:text-amber-300 dark:ring-amber-400/20', 'dot' => 'bg-amber-500 dark:bg-amber-400'],
    ];
    $categoriaStyle = $categoriaStyles[$this->servico->categoria] ?? ['badge' => 'bg-zinc-100 text-zinc-600 ring-1 ring-inset ring-zinc-200 dark:bg-white/10 dark:text-zinc-300 dark:ring-white/10', 'dot' => 'bg-zinc-400 dark:bg-zinc-500'];
    ...
    $tint = $avatarTints[$this->servico->id % count($avatarTints)];
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6 scroll-smooth">
    ...
    <div class="animate-fade-in-up flex flex-wrap items-center gap-3">
        <flux:button
            ...
            variant="ghost"
            size="sm"
            :aria-label="__('Voltar para serviços')"
        />

        <flux:breadcrumbs class="min-w-0 flex-1">
            <flux:breadcrumbs.item :href="route('servicos.index')" wire:navigate>{{ __('Serviços') }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item class="truncate">{{ $this->servico->nome }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>

        <div class="flex shrink-0 items-center gap-2">
            <flux:button href="{{ route('servicos.edit', $this->servico) }}" wire:navigate variant="primary" icon="pencil">
                {{ __('Editar') }}
            </flux:button>
            <flux:button
                wire:click="toggleAtivo"
                variant="ghost"
                :icon="$this->servico->ativo ? 'x-mark' : 'check'"
                :title="$this->servico->ativo ? __('Desativar') : __('Ativar')"
            >
                {{ $this->servico->ativo ? __('Desativar') : __('Ativar') }}
            </flux:button>
            <flux:button
        ...
        <div class="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between lg:gap-10">
            <div class="flex min-w-0 items-center gap-3 sm:gap-4">
                <div class="flex size-12 shrink-0 items-center justify-center rounded-xl text-base font-bold shadow-lg ring-4 ring-black/5 sm:size-16 sm:rounded-2xl sm:text-lg {{ $tint }}">
                    {{ \Illuminate\Support\Str::initials($this->servico->nome, true) }}
                </div>
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        <h2 class="truncate text-xl font-semibold tracking-tight text-zinc-900 sm:text-2xl dark:text-white">{{ $this->servico->nome }}</h2>
                ...
                <div class="rounded-xl bg-white/60 p-3.5 backdrop-blur-sm sm:p-4 dark:bg-white/[0.03]">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="truncate text-sm font-semibold text-zinc-900 dark:text-white">{{ $this->servico->codigo ?: '—' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-xl bg-white/60 p-3.5 backdrop-blur-sm sm:p-4 dark:bg-white/[0.03]">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="truncate text-sm font-semibold text-zinc-900 dark:text-white">{{ $this->servico->preco !== null ? 'R$ '.number_format($this->servico->preco, 2, ',', '.') : '—' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-xl bg-white/60 p-3.5 backdrop-blur-sm sm:p-4 dark:bg-white/[0.03]">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="truncate text-sm font-semibold text-zinc-900 dark:text-white">{{ $this->servico->categoria ?: '—' }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <x-ui.detail-card id="descricao" data-section icon="document-text" :title="__('Descrição')" delay="160ms">
        @if ($this->servico->descricao)
            <p class="whitespace-pre-line text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{{ $this->servico->descricao }}</p>
        @else
    ...
    <x-ui.detail-card id="observacoes" data-section icon="chat-bubble-left-right" :title="__('Observações')" delay="200ms">
        @if ($this->servico->observacoes)
            <p class="whitespace-pre-line text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{{ $this->servico->observacoes }}</p>
        @else
...
@if ($showDeleteModal)
    <flux:modal wire:model="showDeleteModal">
        <flux:heading level="2">{{ __('Excluir serviço') }}</flux:heading>
        <flux:text class="mt-2">
            {{ __('Tem certeza que deseja excluir') }} <strong>{{ $this->servico->nome }}</strong>? {{ __('Esta ação não pode ser desfeita.') }}
        </flux:text>
