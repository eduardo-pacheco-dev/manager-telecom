<div class="flex h-full w-full flex-1 flex-col">
    ...
    <div class="flex min-h-0 flex-1">
        {{-- File tree sidebar --}}
        @if ($showArvore)
            <aside class="hidden w-72 shrink-0 overflow-y-auto border-e border-zinc-200 p-3 lg:block dark:border-white/10">
                ...
                <div class="flex flex-col gap-0.5">
                    ...
                    {{-- Stations --}}
                    @foreach ($this->arvore as $estacao)
                        @php
                            $estacaoChave = 'estacao-'.$estacao['id'];
                            $estacaoAberta = in_array($estacaoChave, $this->expandidos, true) || $this->estacaoId === $estacao['id'];
                            $estacaoAtiva = $this->estacaoId === $estacao['id'] && $this->ordemServicoId === null && $this->radioLinkId === null;
                        @endphp
                                ...
                                <div class="ml-4 border-s border-zinc-200 ps-2 dark:border-white/10">
                                    @foreach ($estacao['filhos'] as $filho)
                                        @php
                                            ...
                                            $filhoAtivo = ($filho['tipo'] === 'radio' && $this->radioLinkId === $filho['id'])
                                                || ($filho['tipo'] === 'ordem' && $this->ordemServicoId === $filho['id']);
                                        @endphp
                                        <button
                                            type="button"
