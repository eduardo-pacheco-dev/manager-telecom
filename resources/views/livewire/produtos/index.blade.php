<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6">
    ...
    <x-ui.page-header
        :title="__('Produtos')"
        :subtitle="__('Gerencie os produtos da empresa')"
        :badge="$this->stats['total']"
        :breadcrumbs="[
            ['label' => __('Gestão'), 'href' => null],
            ['label' => __('Produtos'), 'href' => null],
        ]"
    >
    ...
    {{-- Table --}}
    @php
        $temFiltros = $search !== '' || $filtroCategoria !== '' || $filtroStatus !== '';
    @endphp

    <x-ui.table
        :total="$produtos->total()"
        :first-item="$produtos->firstItem()"
        :last-item="$produtos->lastItem()"
        loading-targets="search,filtroCategoria,filtroStatus,sortBy,perPage,toggleAtivo,destroy"
    ...
    >
            ...
            @if ($produtos->total() > 0)
                <th scope="col" class="px-5 py-3 text-left">
                    <input
                        ...
                        :checked="count(array_intersect($this->selecionados, $produtos->pluck('id')->all())) === $produtos->count()"
                ...
                <th scope="col" class="px-4 py-3 text-left">
                    <x-ui.sortable-header field="nome" :label="__('Produto')" :sort-field="$sortField" :sort-direction="$sortDirection" />
                </th>
                <th scope="col" class="px-4 py-3 text-left">
                    <x-ui.sortable-header field="categoria" :label="__('Categoria / Descrição')" :sort-field="$sortField" :sort-direction="$sortDirection" />
                </th>
                <th scope="col" class="px-4 py-3 text-right">
                    <x-ui.sortable-header field="preco" :label="__('Preço')" align="right" :sort-field="$sortField" :sort-direction="$sortDirection" />
                </th>
                <th scope="col" class="px-4 py-3 text-left">
        ...
        @forelse ($produtos as $produto)
            <x-produtos.row :produto="$produto" />
        @empty
            <tr wire:key="produto-empty">
                <td colspan="6">
                    <x-ui.empty
                        icon="cube"
                        :title="__('Nenhum produto encontrado')"
                    ...
                    >
                        ...
                        @else
                            <flux:button href="{{ route('produtos.create') }}" wire:navigate variant="primary" size="sm" icon="plus">
        ...
        <x-slot:footer>
            <x-ui.pagination :paginator="$produtos" show-per-page :per-page="$perPage" />
        </x-slot:footer>
    </x-ui.table>

    {{-- Delete confirmation modal --}}
    <x-produtos.delete-modal
        :produto-para-excluir="$produtoParaExcluir"
        :produto-alvo="$this->produtoAlvo"
    />

    {{-- Import modal --}}
    <x-produtos.import-modal
