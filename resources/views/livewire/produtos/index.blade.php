<div class="flex h-full w-full flex-1 flex-col gap-6 p-4 sm:p-6">
    ...
    <div class="animate-fade-in-up flex flex-col gap-5">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item class="text-zinc-900 dark:text-white">{{ __('Produtos') }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
                ...
                <flux:subheading size="lg" class="mt-1">{{ __('Gerencie os produtos da empresa') }}</flux:subheading>
            </div>

            <div class="flex items-center gap-2">
                <flux:button href="{{ route('produtos.create') }}" wire:navigate variant="primary" icon="plus">
                    {{ __('Novo Produto') }}
                </flux:button>
            </div>
        </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="animate-fade-in-up rounded-2xl border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-white/5" style="animation-delay: 40ms">
            <div class="flex items-center justify-between gap-3">
                <div class="min-w-0">
                    <p class="truncate text-sm text-zinc-500 dark:text-zinc-400">{{ __('Total de produtos') }}</p>
    ...
    <div class="animate-fade-in-up overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-none dark:border-white/10 dark:bg-white/[0.03]" style="animation-delay: 280ms">
        <div class="flex items-center justify-between border-b border-zinc-200 px-5 py-3.5 dark:border-white/10">
            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                @if ($produtos->total() > 0)
                    {{ __('Mostrando') }}
                    <span class="font-medium text-zinc-900 dark:text-white">{{ $produtos->firstItem() }}-{{ $produtos->lastItem() }}</span>
                    {{ __('de') }}
                    <span class="font-medium text-zinc-900 dark:text-white">{{ $produtos->total() }}</span>
                @else
                    {{ __('Nenhum resultado') }}
                @endif
            </p>
        ...
        <div wire:loading.class="opacity-40" wire:target="search,filtroCategoria,filtroStatus,toggleAtivo" class="transition-opacity duration-200">
            ...
            @if ($produtos->total() > 0)
                <div class="flex items-center gap-4 border-b border-zinc-100 px-5 py-2.5 text-xs font-medium uppercase tracking-wider text-zinc-400 dark:border-white/5 dark:text-zinc-500">
                    <div class="min-w-0 flex-1">{{ __('Produto') }}</div>
            ...
            @forelse ($produtos as $produto)
                ...
                <div class="group flex items-center gap-4 border-b border-zinc-100 px-5 py-4 transition-all duration-200 last:border-b-0 hover:bg-zinc-50/80 dark:border-white/5 dark:hover:bg-white/[0.02]">
                    {{-- Avatar + Name --}}
                    <a href="{{ route('produtos.show', $produto) }}" wire:navigate class="flex min-w-0 flex-1 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <p class="truncate text-sm font-medium text-zinc-900 group-hover:text-sky-600 dark:text-white dark:group-hover:text-sky-400">{{ $produto->nome }}</p>
                    ...
                    <div class="hidden w-28 text-right md:block">
                        @if ($produto->preco !== null)
                            <span class="text-xs font-medium text-zinc-700 dark:text-zinc-300">R$ {{ number_format($produto->preco, 2, ',', '.') }}</span>
                        @else
                    ...
                    <div class="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                        <flux:button
                            ...
                            :aria-label="__('Ver detalhes de') . ' ' . $produto->nome"
                        />
                        <flux:button
                            ...
                            icon="pencil-square"
                            :title="__('Editar')"
                            :aria-label="__('Editar') . ' ' . $produto->nome"
                        />
                        <flux:button
                            ...
                            :title="$produto->ativo ? __('Desativar') : __('Ativar')"
                            :aria-label="$produto->ativo ? __('Desativar') . ' ' . $produto->nome : __('Ativar') . ' ' . $produto->nome"
                        />
                    </div>
                </div>
            @empty
                <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                    <div class="flex size-14 items-center justify-center rounded-full bg-zinc-100 dark:bg-white/10">
                        <flux:icon.cube class="size-7 text-zinc-400 dark:text-zinc-500" />
                    </div>
                    ...
                    @else
                        <flux:button href="{{ route('produtos.create') }}" wire:navigate variant="primary" size="sm" icon="plus" class="mt-5">
                            {{ __('Cadastrar primeiro produto') }}
                        </flux:button>
                    @endif
                </div>
            @endforelse
        ...
        @if ($produtos->hasPages())
            <div class="flex items-center justify-between border-t border-zinc-200 px-5 py-3 dark:border-white/10">
                <div class="text-xs text-zinc-500 dark:text-zinc-400">
                    {{ __('Mostrando') }} {{ $produtos->firstItem() }} {{ __('a') }} {{ $produtos->lastItem() }} {{ __('de') }} {{ $produtos->total() }} {{ __('resultados') }}
                </div>
                <div class="flex items-center gap-1">
                    ...
                    @foreach ($produtos->getUrlRange(max(1, $produtos->currentPage() - 1), min($produtos->lastPage(), $produtos->currentPage() + 1)) as $page => $url)
