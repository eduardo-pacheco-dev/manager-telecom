...
<flux:modal wire:model="showImportModal" class="max-w-2xl">
    <div class="space-y-5">
        <div>
            <flux:heading level="2">{{ __('Importar estações') }}</flux:heading>
            <flux:text class="mt-2">
                {{ __('Envie um arquivo Excel (.xlsx) ou CSV com as estações. A importação roda em segundo plano via fila de jobs, ideal para arquivos com milhares de linhas.') }}
            </flux:text>
        </div>
        ...
        <div class="rounded-xl border border-zinc-100 bg-zinc-50/60 p-4 dark:border-white/5 dark:bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3">
                ...
                <a
                    href="{{ route('estacoes.importar.modelo') }}"
            ...
            <div class="mt-1.5 flex flex-wrap gap-1.5">
                @foreach (['Site_ID', 'Endereco_ID', 'Tecnologia', 'Operadora', 'Tipo_Conexao', 'Station_ID', 'Municipio', 'Estado', 'CEP', 'Regional', 'Status', 'Detentor_Area', 'Tipo_Infra', 'Tipo_EV', 'Latitude', 'Longitude', 'Tipo_Logradouro', 'Logradouro', 'Numero', 'Complemento', 'Bairro', 'Tipo_Torre', 'AEV_Nominal', 'Area_Solo', 'Altura_Estrutura'] as $coluna)
            ...
            <p class="mt-2 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                {{ __('Obrigatória') }}: <strong>Site_ID</strong>.
                {{ __('Linhas com Site ID duplicado são ignoradas.') }}
            </p>
        </div>
