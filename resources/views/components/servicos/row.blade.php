...
@php
    ...
    $categoriaStyle = [
        'Instalação' => 'bg-sky-500/10 text-sky-700 ring-1 ring-inset ring-sky-500/20 dark:bg-sky-400/10 dark:text-sky-300 dark:ring-sky-400/20',
        'Manutenção' => 'bg-violet-500/10 text-violet-700 ring-1 ring-inset ring-violet-500/20 dark:bg-violet-400/10 dark:text-violet-300 dark:ring-violet-400/20',
        'Suporte' => 'bg-emerald-500/10 text-emerald-700 ring-1 ring-inset ring-emerald-500/20 dark:bg-emerald-400/10 dark:text-emerald-300 dark:ring-emerald-400/20',
        'Configuração' => 'bg-amber-500/10 text-amber-700 ring-1 ring-inset ring-amber-500/20 dark:bg-amber-400/10 dark:text-amber-300 dark:ring-amber-400/20',
    ];
    $categoriaStyle = $categoriaStyle[$servico->categoria] ?? 'bg-zinc-100 text-zinc-600 ring-1 ring-inset ring-zinc-200 dark:bg-white/10 dark:text-zinc-300 dark:ring-white/10';
@endphp

<tr wire:key="servico-{{ $servico->id }}" class="group border-b border-zinc-100 transition-colors last:border-b-0 hover:bg-zinc-50/70 dark:border-white/5 dark:hover:bg-white/[0.02]">
    {{-- Seleção --}}
    <td class="whitespace-nowrap px-5 py-3.5 align-middle">
        <input
            ...
            value="{{ $servico->id }}"
            aria-label="{{ __('Selecionar') . ' ' . $servico->nome }}"
    ...
    <td class="whitespace-nowrap px-4 py-3.5 align-middle">
        <a href="{{ route('servicos.show', $servico) }}" wire:navigate class="group/link flex min-w-0 items-center gap-3">
            ...
            <div class="min-w-0">
                <p class="truncate text-sm font-medium text-zinc-900 transition-colors group-hover/link:text-sky-600 dark:text-white dark:group-hover/link:text-sky-400">{{ $servico->nome }}</p>
        ...
        <div class="flex flex-col items-start gap-1">
            @if ($servico->categoria)
                <span class="inline-flex max-w-full items-center rounded-full px-2.5 py-1 text-xs font-medium {{ $categoriaStyle }}">
                    <span class="truncate">{{ $servico->categoria }}</span>
                </span>
            @endif
    ...
    <td class="whitespace-nowrap px-5 py-3.5 text-right align-middle">
        <div class="inline-flex items-center justify-end">
            <x-servicos.row-actions :servico="$servico" />
        </div>
    </td>
</tr>