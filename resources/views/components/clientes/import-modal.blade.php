...
<flux:modal wire:model="showImportModal" class="max-w-2xl">
    <div class="space-y-5">
        <div>
            <flux:heading level="2">{{ __('Importar clientes') }}</flux:heading>
            <flux:text class="mt-2">
                {{ __('Envie um arquivo Excel (.xlsx) ou CSV com os clientes. A importação roda em segundo plano via fila de jobs, ideal para arquivos com milhares de linhas.') }}
            </flux:text>
        </div>
        ...
        <div class="rounded-xl border border-zinc-100 bg-zinc-50/60 p-4 dark:border-white/5 dark:bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3">
                ...
                <a
                    href="{{ route('clientes.importar.modelo') }}"
            ...
            <div class="mt-1.5 flex flex-wrap gap-1.5">
                @foreach (['Nome', 'Email', 'CNPJ', 'Telefone', 'Segmento', 'Cidade', 'Estado', 'Status'] as $coluna)
            ...
            <p class="mt-2 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                {{ __('Obrigatórias') }}: <strong>Nome</strong>, <strong>Email</strong> e <strong>CNPJ</strong>.
                {{ __('Linhas duplicadas (e-mail ou CNPJ) são ignoradas.') }}
            </p>
        </div>
