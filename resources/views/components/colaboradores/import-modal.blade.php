...
<flux:modal wire:model="showImportModal" class="max-w-2xl">
    <div class="space-y-5">
        <div>
            <flux:heading level="2">{{ __('Importar colaboradores') }}</flux:heading>
            <flux:text class="mt-2">
                {{ __('Envie um arquivo Excel (.xlsx) ou CSV com os colaboradores. A importação roda em segundo plano via fila de jobs, ideal para arquivos com milhares de linhas.') }}
            </flux:text>
        </div>
        ...
        <div class="rounded-xl border border-zinc-100 bg-zinc-50/60 p-4 dark:border-white/5 dark:bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3">
                ...
                <a
                    href="{{ route('colaboradores.importar.modelo') }}"
            ...
            <div class="mt-1.5 flex flex-wrap gap-1.5">
                @foreach (['Nome', 'Email', 'CPF', 'Telefone', 'Cargo', 'Departamento', 'Categoria', 'Data_Admissão', 'Salário', 'Status'] as $coluna)
            ...
            <p class="mt-2 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                {{ __('Obrigatórias') }}: <strong>Nome</strong>, <strong>Email</strong> e <strong>CPF</strong>.
                {{ __('Linhas duplicadas (e-mail ou CPF) são ignoradas. A categoria é normalizada para CLT, PJ ou Freelancer.') }}
            </p>
        </div>
