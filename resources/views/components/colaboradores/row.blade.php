...
@php
    ...
    $categoriaStyle = [
        'CLT' => 'bg-sky-500/10 text-sky-700 ring-1 ring-inset ring-sky-500/20 dark:bg-sky-400/10 dark:text-sky-300 dark:ring-sky-400/20',
        'PJ' => 'bg-violet-500/10 text-violet-700 ring-1 ring-inset ring-violet-500/20 dark:bg-violet-400/10 dark:text-violet-300 dark:ring-violet-400/20',
        'Freelancer' => 'bg-amber-500/10 text-amber-700 ring-1 ring-inset ring-amber-500/20 dark:bg-amber-400/10 dark:text-amber-300 dark:ring-amber-400/20',
    ];
    $categoriaStyle = $categoriaStyle[$colaborador->categoria] ?? 'bg-zinc-100 text-zinc-600 ring-1 ring-inset ring-zinc-200 dark:bg-white/10 dark:text-zinc-300 dark:ring-white/10';
@endphp
...
<tr wire:key="colab-{{ $colaborador->id }}" class="group border-b border-zinc-100 transition-colors last:border-b-0 hover:bg-zinc-50/70 dark:border-white/5 dark:hover:bg-white/[0.02]">
    ...
    <td class="whitespace-nowrap px-4 py-3.5 align-middle">
        <a href="{{ route('colaboradores.show', $colaborador) }}" wire:navigate class="group/link flex min-w-0 items-center gap-3">
            ...
            <div class="min-w-0">
                <p class="truncate text-sm font-medium text-zinc-900 transition-colors group-hover/link:text-sky-600 dark:text-white dark:group-hover/link:text-sky-400">{{ $colaborador->nome }}</p>
    ...
    <td class="whitespace-nowrap px-5 py-3.5 text-right align-middle">
        <div class="inline-flex items-center justify-end">
            <x-colaboradores.row-actions :colaborador="$colaborador" />
        </div>
    </td>
</tr>