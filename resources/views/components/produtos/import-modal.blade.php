...
<flux:modal wire:model="showImportModal" class="max-w-2xl">
    <div class="space-y-5">
        <div>
            <flux:heading level="2">{{ __('Importar produtos') }}</flux:heading>
            <flux:text class="mt-2">
                {{ __('Envie um arquivo Excel (.xlsx) ou CSV com os produtos. A importação roda em segundo plano via fila de jobs, ideal para arquivos com milhares de linhas.') }}
            </flux:text>
        </div>
        ...
        <div class="rounded-xl border border-zinc-100 bg-zinc-50/60 p-4 dark:border-white/5 dark:bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3">
                ...
                <a
                    href="{{ route('produtos.importar.modelo') }}"
            ...
            <div class="mt-1.5 flex flex-wrap gap-1.5">
                @foreach (['Nome', 'Código', 'Categoria', 'Descrição', 'Preço', 'Status'] as $coluna)
            ...
            <p class="mt-2 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                {{ __('Obrigatória') }}: <strong>Nome</strong>.
                {{ __('A categoria é normalizada para Equipamento, Infraestrutura, Acessório ou Cabeamento. Linhas com código duplicado são ignoradas.') }}
            </p>
        </div>
