...
<flux:modal wire:model="showImportModal" class="max-w-2xl">
    <div class="space-y-5">
        <div>
            <flux:heading level="2">{{ __('Importar projetos TIM Implantação RF') }}</flux:heading>
            <flux:text class="mt-2">
                {{ __('Envie um arquivo Excel (.xlsx) ou CSV com os projetos. A importação roda em segundo plano via fila de jobs, ideal para arquivos com milhares de linhas.') }}
            </flux:text>
        </div>
        ...
        <div class="rounded-xl border border-zinc-100 bg-zinc-50/60 p-4 dark:border-white/5 dark:bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3">
                ...
                <a
                    href="{{ route('tim.importar.modelo') }}"
            ...
            <div class="mt-1.5 flex flex-wrap gap-1.5">
                @foreach (['Código', 'Descrição', 'Status', 'Cliente', 'Estação', 'OC', 'OS FAM Entrega', 'OS FAM Instalação', 'OS FAM Panorâmica', 'OS FAM Desinstalação', 'Data Início', 'Data Fim', 'Ativo', 'Criar OS'] as $coluna)
            ...
            <p class="mt-2 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                {{ __('Obrigatória') }}: <strong>Código</strong>.
                {{ __('O cliente é resolvido pelo nome e a estação pelo Site ID. Marque "Criar OS" como "Sim" para gerar uma ordem de serviço automaticamente. Linhas sem código são ignoradas.') }}
            </p>
        </div>
